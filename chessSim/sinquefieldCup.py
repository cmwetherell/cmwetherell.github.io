...
def getPlayers(): 
    ...
    gctPlayers = gctPlayers[gctPlayers.Name.isin(['Carlsen','Firouzja', 'So', 'Caruana', 'Nepomniachtchi', 'Mamedyarov', 'Dominguez Perez', 'Aronian', 'Vachier-Lagrave' ])]
    gctPlayers = {x[0]: Player(x[0], x[1], x[2] , x[3]) for x in np.array(gctPlayers)}
    gctPlayers['Niemann'] = Player('Niemann', 2703, 2603, 2632)
...
# tournament = SCup(getPlayers(), current)
# tournament.simCup()
