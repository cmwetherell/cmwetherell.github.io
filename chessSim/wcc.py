...
def getPlayers(): 
    players = pickle.load(open( "./chessSim/data/playerData.p", "rb" ) )
    players = players[players.Name.isin(
        ['Gukesh D',
         'Ding, Liren',
         ])]
    players = {x[0]: Player(x[0], x[1], x[2] , x[3]) for x in np.array(players)}

    players['Ding, Liren'].EloC = 2738
    players['Gukesh D'].EloC = 2773
...
class WCC:
    ...
    def createGames(self):
        ...
        # Multiply the list by 7 to repeat each game 7 times
        rrGames = rrGames * 7
        # rrGames.sort()
        ...
        if len(s1Players) > 1:
            if len(s1Players) == 2:
                ...
                if stage == 's1':
                    ...
                    self.games.loc[self.games.shape[0]] = [player1.name, player2.name, 'r', 's1', 1, game1, 1*(game1==0), 0]

                    game2 = playChess(bst, player2, player1, format = 'r') #points white player scored
                    self.games.loc[self.games.shape[0]] = [player2.name, player1.name, 'r', 's1', 1, game2, 1*(game1==0), 0]
