...
<small><em>Lift charts for the four production candidates. Each chart sorts positions by the model's predicted complexity (x-axis) and shows the actual complexity in each decile (black line). A good model produces a clean upward staircase — positions it predicts as complex actually are complex.</em></small>
